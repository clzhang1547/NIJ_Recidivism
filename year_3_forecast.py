...
dt = get_df_for_forecast('./data/nij/NIJ_s_Recidivism_Challenge_Test_Dataset3.csv')
...
#############################################
# Year 3 - subgroup cannot handle rare cases?
# Use one model (best is Logit L2) for all
#############################################
# df = d.copy()
...
# phat_out.to_csv('./output/year_3_forecast/year_3_forecast.csv', index=False)
# phat_out.to_excel('./_management/_submission/Year3/AMERICAN INSTITUTES FOR RESEARCH_3YearForecast.xlsx', index=False)
#

########################################################
# Year 3 Model Training with Tuned Parameters
########################################################
...
# Model
pipe = Pipeline([("scale", StandardScaler()),
                 ("reduce_dims", PCA(n_components=0.5)),
                 ('model', clf)])
# quick validation
# scores = ['roc_auc', 'f1', 'precision', 'recall', 'accuracy', 'neg_brier_score']
# dct_score = cross_validate(pipe, X, y, cv=5, scoring=scores)
