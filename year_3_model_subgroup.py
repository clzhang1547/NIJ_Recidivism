...
# Year 3 model - based on subgroup analysis
...
dct_subgroup_master = OrderedDict(sorted(dct_subgroup_master.items(), key=lambda x: x[1]['brier'],
                                             reverse=True))
fp_out = './output/year_3_forecast/dct_subgroup_master.json'  # group_id defined when subgrouping d
with open(fp_out, 'w') as f:
    json.dump(dct_subgroup_master, f, indent=4)

# get subgrouping
df = d.copy()
df = df[(df['recidivism_arrest_year1']==0) & (df['recidivism_arrest_year2']==0)]
